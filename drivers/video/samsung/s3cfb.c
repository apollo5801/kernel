...
{
	printk("s3cfb_set_bitfield debut : bpp = %d\n", var->bits_per_pixel);
	int bpp = var->bits_per_pixel;
	switch (bpp)
	{
		case 16:
			printk("bitfields 16bpp\n");
...
static int s3cfb_set_alpha_info(struct fb_var_screeninfo *var, struct s3cfb_window *win)
{
	...
	printk("s3cfb_set_alpha_info fin : tl = %d\n", var->transp.length);
	return 0;
}
...
int s3cfb_direct_ioctl(int id, unsigned int cmd, unsigned long arg)
{
	...
	void *argp = (void *) arg;
	int ret = 0;
	printk("s3cfb_direct_ioctl debut\n");
	switch (cmd) {
	case FBIO_ALLOC:
